...
public interface PrescriptionRepository extends JpaRepository<Prescription,Long>{
	...
	@Query(value="select p.prescription_code as prescriptionCode,p.prescription_id, p.prescription_date, p.doctor_id from prescription p where p.patient_id=?1 and p.prescription_status=?2",nativeQuery=true)
	...
	@Modifying
	@Transactional
	@Query(value="update prescription p set p.prescription_status='used' where p.prescription_id = ?1 and p.prescription_code = ?2",nativeQuery=true)
	int confirmPrescriptionById(long id, int code);
